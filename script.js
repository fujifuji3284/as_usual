$(function(){
	...
	$(window).resize(function(){
		...
		$("div#header").css({width:w+"px"});
		$("div#header").css({height:w/1.4375+"px"});
		$("div#header > div span").css({fontSize:w/14+"px"});
	});
	$("#tuka").css({top:w-250+"px",left:"35%"});
	$("#u").css({top:w-250+"px",left:"40%"});
	setTimeout(function(){
		$("#kara").css({top:"40%",left:"25%",opacity:"0.4"}).fadeIn(5000);
		$("#mi").animate({top:"-50px",left:"-90px"},600).animate({top:"-70px",left:"50px"},600).animate({top:"30%",left:"14%"},800).css({opacity:"0.4"});
		...
		$("#he").css({top:"50%",left:"46%"}).fadeIn(4000);
	},4550);
	setTimeout(function(){
		$("#tuka").animate({top:"50%",left:"19%"},800);
